...
enum SettingsTab: String, CaseIterable, Identifiable {
    ...
    var title: String {
        ...
        case .macros: return "语音宏"
        case .runLog: return "运行日志"
...
enum ASREngineMode: String, CaseIterable, Identifiable {
    case qwenNative = "qwen_native"
    case senseVoiceGemma = "sensevoice_gemma"
    ...
    var title: String {
        switch self {
        case .qwenNative:
            return "Qwen3-ASR + Gemma E4B"
        case .senseVoiceGemma:
            return "SenseVoice + Gemma E4B"
        case .thirdPartyAPI:
            return "第三方 API"
        }
    }
    ...
    var subtitle: String {
        switch self {
        case .qwenNative:
            return "Swift 进程内运行 Qwen3-ASR 识别，本地 Gemma E4B 应用提示词、纠错和常用词库。"
        case .senseVoiceGemma:
            return "启动本地 Python 后端，使用 SenseVoice 识别并用 Gemma E4B 纠错。"
        case .thirdPartyAPI:
            return "音频识别与文本纠错走 OpenAI-compatible API 配置。"
        }
    }
...
enum AppBuild {
    static var isDevBundle: Bool {
        (Bundle.main.object(forInfoDictionaryKey: "CFBundleName") as? String) == "OPEN SPEECH"
    }
}
...
final class AppState: ObservableObject, @unchecked Sendable {
    ...
    private let localModeStorageKey = "local_mode_enabled"
    private let asrEngineModeStorageKey = "asr_engine_mode"
    private let customLLMPostProcessingEnabledStorageKey = "custom_llm_post_processing_enabled"
    private let customLLMContextLimitStorageKey = "custom_llm_context_limit"
    private let onboardingCompletedStorageKey = "onboarding_completed_v1"
    private let launchAtLoginDesiredStorageKey = "launch_at_login_desired"
    private let systemPromptPresetNamesStorageKey = "system_prompt_preset_names"
    ...
    static let defaultTranscriptionModel = "qwen3-asr-1.7b-mlx-4bit"
    ...
    static let defaultProviderAPIBaseURL = "http://127.0.0.1:8001/v1"
    static let defaultCustomLLMContextLimit = 8192
    static let minCustomLLMContextLimit = 512
    static let maxCustomLLMContextLimit = 100000
    private static let promptPresetDateFormatter: DateFormatter = {
    ...
    private static var defaultSystemPromptPresetNames: [String: String] {
        Dictionary(uniqueKeysWithValues: SystemPromptPreset.allCases.map { ($0.rawValue, $0.defaultName) })
    }

    private static var defaultSystemPromptPresetPrompts: [String: String] {
        Dictionary(uniqueKeysWithValues: SystemPromptPreset.allCases.map { ($0.rawValue, $0.defaultPrompt) })
    }
    ...
    @Published var hasCompletedOnboarding: Bool {
        didSet {
            UserDefaults.standard.set(hasCompletedOnboarding, forKey: onboardingCompletedStorageKey)
        }
    }
    ...
    @Published var customLLMContextLimit: Int {
        didSet {
            let normalized = Self.normalizedCustomLLMContextLimit(customLLMContextLimit)
            if normalized != customLLMContextLimit {
                customLLMContextLimit = normalized
                return
            }
            UserDefaults.standard.set(normalized, forKey: customLLMContextLimitStorageKey)
        }
    }
    ...
    @Published var systemPromptPresetNames: [String: String] = [:] {
        didSet {
            UserDefaults.standard.set(systemPromptPresetNames, forKey: systemPromptPresetNamesStorageKey)
        }
    }

    @Published var systemPromptPresetPrompts: [String: String] = [:] {
        didSet {
            UserDefaults.standard.set(systemPromptPresetPrompts, forKey: systemPromptPresetPromptsStorageKey)
        }
    }

    @Published var selectedSystemPromptPreset: SystemPromptPreset = .a {
        didSet {
            UserDefaults.standard.set(selectedSystemPromptPreset.rawValue, forKey: selectedSystemPromptPresetStorageKey)
        }
    }
    ...
    @Published var qwen3ASRLoadState: Qwen3ASRLoadState = .idle
    ...
    @Published var isLocalMode: Bool = true {
        didSet {
            UserDefaults.standard.set(isLocalMode, forKey: localModeStorageKey)
            NotificationCenter.default.post(name: .localBackendModeChanged, object: nil)
        }
    }

    @Published var customLLMPostProcessingEnabled: Bool {
        didSet {
            UserDefaults.standard.set(
                customLLMPostProcessingEnabled,
                forKey: customLLMPostProcessingEnabledStorageKey
            )
            NotificationCenter.default.post(name: .localBackendModeChanged, object: nil)
        }
    }

    @Published var hasScreenRecordingPermission = true
    @Published var launchAtLogin: Bool {
        didSet {
            guard !isUpdatingLaunchAtLoginFromSystem else { return }
    ...
    var shouldRunSenseVoiceASRBackend: Bool {
        asrEngineMode == .senseVoiceGemma
    }

    var customLLMConfigurationMessage: String? {
        customLLMConfigurationError()
    }

    var hasCustomLLMPostProcessingConfiguration: Bool {
        customLLMConfigurationError() == nil
    }

    init() {
        UserDefaults.standard.removeObject(forKey: "force_http2_transcription")
        let hasCompletedSetup = true
        let storedAPIKey = Self.loadStoredAPIKey(account: apiKeyStorageKey)
        let apiKey = storedAPIKey.isEmpty ? "local" : storedAPIKey
        ...
        let transcriptionLanguage = Self.normalizeTranscriptionLanguage(
            UserDefaults.standard.string(forKey: transcriptionLanguageStorageKey) ?? "zh"
        )
        var customSystemPrompt = UserDefaults.standard.string(forKey: customSystemPromptStorageKey) ?? ""
        let systemPromptPresetNames = Self.loadSystemPromptPresetDictionary(
            key: systemPromptPresetNamesStorageKey,
            defaults: Self.defaultSystemPromptPresetNames
        )
        var systemPromptPresetPrompts = Self.loadSystemPromptPresetDictionary(
            key: systemPromptPresetPromptsStorageKey,
            defaults: Self.defaultSystemPromptPresetPrompts
        )
        let selectedSystemPromptPreset = SystemPromptPreset(
            rawValue: UserDefaults.standard.string(forKey: selectedSystemPromptPresetStorageKey) ?? ""
        ) ?? .a
        ...
        let storedCustomLLMContextLimit = UserDefaults.standard.object(forKey: customLLMContextLimitStorageKey) != nil
            ? UserDefaults.standard.integer(forKey: customLLMContextLimitStorageKey)
            : Self.defaultCustomLLMContextLimit
        let customLLMContextLimit = Self.normalizedCustomLLMContextLimit(storedCustomLLMContextLimit)
        ...
        let storedLocalMode = UserDefaults.standard.object(forKey: localModeStorageKey) == nil
            ? true
            : UserDefaults.standard.bool(forKey: localModeStorageKey)
        let storedCustomLLMPostProcessingEnabled = UserDefaults.standard.object(
            forKey: customLLMPostProcessingEnabledStorageKey
        ) == nil
            ? true
            : UserDefaults.standard.bool(forKey: customLLMPostProcessingEnabledStorageKey)
        var asrEngineMode = ASREngineMode(
            rawValue: UserDefaults.standard.string(forKey: asrEngineModeStorageKey) ?? ""
        )
        if asrEngineMode == nil,
           Self.shouldMigrateToHybridLocalMode(
            isLocalMode: storedLocalMode,
            apiBaseURL: apiBaseURL,
            transcriptionAPIURL: transcriptionAPIURL,
            transcriptionModel: transcriptionModel,
            postProcessingModel: postProcessingModel
        ) {
            asrEngineMode = .thirdPartyAPI
        }
        let resolvedASREngineMode = asrEngineMode ?? Self.migratedASREngineMode(
            storedLocalMode: storedLocalMode,
            customLLMPostProcessingEnabled: storedCustomLLMPostProcessingEnabled
        )
        let isLocalMode = resolvedASREngineMode != .thirdPartyAPI
        let customLLMPostProcessingEnabled = resolvedASREngineMode == .thirdPartyAPI
        if UserDefaults.standard.string(forKey: asrEngineModeStorageKey) == nil {
            UserDefaults.standard.set(resolvedASREngineMode.rawValue, forKey: asrEngineModeStorageKey)
        }
        ...
        if !UserDefaults.standard.bool(forKey: systemPromptPresetMigrationV2StorageKey) {
            let mistakenAValue = systemPromptPresetPrompts[SystemPromptPreset.a.rawValue]?
                .trimmingCharacters(in: .whitespacesAndNewlines)
            let classicalPrompt = SystemPromptPreset.d.defaultPrompt.trimmingCharacters(in: .whitespacesAndNewlines)
            if mistakenAValue == classicalPrompt {
                systemPromptPresetPrompts[SystemPromptPreset.a.rawValue] = SystemPromptPreset.a.defaultPrompt
                if selectedSystemPromptPreset == .a &&
                    customSystemPrompt.trimmingCharacters(in: .whitespacesAndNewlines) == classicalPrompt {
                    customSystemPrompt = SystemPromptPreset.a.defaultPrompt
                }
            }
            UserDefaults.standard.set(true, forKey: systemPromptPresetMigrationV2StorageKey)
        }
        if !customSystemPrompt.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty,
           UserDefaults.standard.object(forKey: systemPromptPresetPromptsStorageKey) == nil {
            systemPromptPresetPrompts[selectedSystemPromptPreset.rawValue] = customSystemPrompt
        }
        let resolvedSystemPrompt = customSystemPrompt.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty
            ? (systemPromptPresetPrompts[selectedSystemPromptPreset.rawValue] ?? selectedSystemPromptPreset.defaultPrompt)
            : customSystemPrompt
        ...
        self.customLLMContextLimit = customLLMContextLimit
        ...
        self.customSystemPrompt = resolvedSystemPrompt
        self.systemPromptPresetNames = systemPromptPresetNames
        self.systemPromptPresetPrompts = systemPromptPresetPrompts
        self.selectedSystemPromptPreset = selectedSystemPromptPreset
        ...
        self.asrEngineMode = resolvedASREngineMode
        self.isLocalMode = isLocalMode
        self.customLLMPostProcessingEnabled = customLLMPostProcessingEnabled
    ...
    func performLocalASRMemoryMaintenance(
        ...
        reason: String,
        force: Bool = false
    ) async -> LocalMLXCleanupResult {
        await asrProvider.performMemoryMaintenance(
            reason: reason,
    ...
    func releaseLocalASRModelForMemoryPressure(reason: String) async -> LocalMLXCleanupResult {
        let result = await asrProvider.releaseModelForMemoryPressure(reason: reason)
        await MainActor.run { [weak self] in
            self?.qwen3ASRLoadState = .idle
        }
        return result
    }
    ...
    static func normalizedCustomLLMContextLimit(_ value: Int) -> Int {
        min(max(value, minCustomLLMContextLimit), maxCustomLLMContextLimit)
    }
    ...
    private func customLLMConfigurationError() -> String? {
        if apiBaseURL.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty {
            return "请先填写自定义 LLM API Base URL"
        }
        if postProcessingModel.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty {
            return "请先填写自定义 LLM Model ID"
        }
        return nil
    }
    ...
    private static func shouldMigrateToHybridLocalMode(
        isLocalMode: Bool,
        apiBaseURL: String,
        ...
        guard !isLocalMode else { return false }
        guard transcriptionAPIURL.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty else {
            return false
        }
        guard transcriptionModel.trimmingCharacters(in: .whitespacesAndNewlines) == defaultTranscriptionModel else {
            return false
        }

        guard let components = URLComponents(string: apiBaseURL.trimmingCharacters(in: .whitespacesAndNewlines)) else {
            return false
        }
        ...
        let model = postProcessingModel.trimmingCharacters(in: .whitespacesAndNewlines).lowercased()
        return components.port == 8000 || model.contains("deepseek")
    }

    private static func migratedASREngineMode(
        storedLocalMode: Bool,
        customLLMPostProcessingEnabled: Bool
    ) -> ASREngineMode {
        if !storedLocalMode {
            return .thirdPartyAPI
        }
        if !customLLMPostProcessingEnabled {
            return .senseVoiceGemma
        }
        return .qwenNative
    }

    private static func loadSystemPromptPresetDictionary(
        key: String,
        defaults: [String: String]
    ) -> [String: String] {
        var result = defaults
        if let stored = UserDefaults.standard.dictionary(forKey: key) as? [String: String] {
            for (preset, value) in stored where SystemPromptPreset(rawValue: preset) != nil {
                result[preset] = value
            }
        }
        return result
    ...
    func transcribeAudioForCurrentMode(fileURL: URL) async throws -> String {
        switch asrEngineMode {
        case .qwenNative:
            await MainActor.run {
                if qwen3ASRLoadState != .ready {
                    qwen3ASRLoadState = .loading(message: "Qwen3-ASR 模型准备中")
                }
            }
            let result = try await asrProvider.transcribe(audioURL: fileURL)
            await MainActor.run {
                qwen3ASRLoadState = .ready
            }
            return result.text
        case .senseVoiceGemma:
    ...
    /// URL of the flag file written while OPEN SPEECH is actively recording.
    ///
    ...
    ///
    /// Path: `~/Library/Application Support/OPEN SPEECH/is-recording`
    static func recordingStateFlagURL() -> URL {
        let appSupport = FileManager.default.urls(for: .applicationSupportDirectory, in: .userDomainMask).first!
        let appName = Bundle.main.object(forInfoDictionaryKey: "CFBundleName") as? String ?? "OPEN SPEECH"
    ...
    private static let recordingStateFlagQueue = DispatchQueue(
        label: "com.openspeech.app.recording-state-flag"
    )
    ...
    func retryTranscription(item: PipelineHistoryItem) {
        ...
        let postProcessingService = PostProcessingService(
            ...
            contextLimit: customLLMContextLimit
        )
        ...
        Task {
            do {
                let rawTranscript = try await self.transcribeAudioForCurrentMode(fileURL: audioURL)
                ...
                if self.asrEngineMode == .qwenNative {
                    let result = await self.processTranscriptWithLocalGemma(
                ...
                } else if self.asrEngineMode == .senseVoiceGemma {
                    let result = await self.processTranscriptWithLocalGemma(
                        parsedTranscript.transcript,
                        intent: restoredIntent,
                        context: restoredContext,
    ...
    private func setLaunchAtLogin(_ enabled: Bool) {
        UserDefaults.standard.set(enabled, forKey: launchAtLoginDesiredStorageKey)
        do {
            if enabled {
    ...
    func refreshLaunchAtLoginStatus() {
        ...
        if current != launchAtLogin {
            isUpdatingLaunchAtLoginFromSystem = true
            launchAtLogin = current
            isUpdatingLaunchAtLoginFromSystem = false
        }
    }
    ...
    func activeSystemPromptPreset() -> SystemPromptPreset? {
        selectedSystemPromptPreset
    }

    func applySystemPromptPreset(_ preset: SystemPromptPreset) {
        selectedSystemPromptPreset = preset
        let newPrompt = systemPromptPresetPrompt(for: preset)
        if customSystemPrompt != newPrompt {
            customSystemPrompt = newPrompt
        }

        let newModifiedDate = Self.promptPresetDateFormatter.string(from: Date())
        if customSystemPromptLastModified != newModifiedDate {
            customSystemPromptLastModified = newModifiedDate
        }
    }

    func systemPromptPresetName(for preset: SystemPromptPreset) -> String {
        let stored = systemPromptPresetNames[preset.rawValue]?.trimmingCharacters(in: .whitespacesAndNewlines) ?? ""
        return stored.isEmpty ? preset.defaultName : stored
    }

    func renameSystemPromptPreset(_ preset: SystemPromptPreset, to name: String) {
        var names = systemPromptPresetNames
        names[preset.rawValue] = name.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty
            ? preset.defaultName
            : name
        systemPromptPresetNames = names
    }

    func systemPromptPresetPrompt(for preset: SystemPromptPreset) -> String {
        systemPromptPresetPrompts[preset.rawValue] ?? preset.defaultPrompt
    }

    func updateSelectedSystemPromptPresetPrompt(_ prompt: String) {
        updateSystemPromptPresetPrompt(selectedSystemPromptPreset, to: prompt)
    }

    func updateSystemPromptPresetPrompt(_ preset: SystemPromptPreset, to prompt: String) {
        var prompts = systemPromptPresetPrompts
        prompts[preset.rawValue] = prompt
        systemPromptPresetPrompts = prompts
    }
    ...
    private func beginCriticalDictationActivity() {
        ...
        ProcessInfo.processInfo.disableAutomaticTermination("OPEN SPEECH dictation in progress")
        automaticTerminationDisabled = true
    }

    private func endCriticalDictationActivity() {
        guard automaticTerminationDisabled else { return }
        ProcessInfo.processInfo.enableAutomaticTermination("OPEN SPEECH dictation in progress")
        ...
        DispatchQueue.global(qos: .userInitiated).async { [weak self] in
            ...
            do {
                ...
                DispatchQueue.main.async {
                    ...
                    // MLX Voice Input: context capture disabled for speed
    ...
    private enum TranscriptProcessingOutcome {
        ...
        case asrOnly(engine: String)
        case postProcessingSucceeded
        case postProcessingFailedFallback
        case postProcessingConfigurationMissing(String)
        ...
        func statusMessage(isRetry: Bool = false) -> String {
            ...
            case .asrOnly(let engine):
                return "\(engine) ASR only"
            case .postProcessingSucceeded:
            ...
            case .postProcessingConfigurationMissing(let message):
                return "Post-processing skipped: \(message)"
    ...
    private func processTranscriptWithLocalGemma(
        _ rawTranscript: String,
        intent: SessionIntent,
        context: AppContext,
        customVocabulary: String,
        ...
        if case .command(let invocation, let selectedText) = intent {
            do {
                ...
                """
                let result = try await LocalBackendClient.polish(
                    text: commandInput,
                ...
                return (
                    ...
                    localGemmaPromptForDisplay(
                        systemPrompt: PostProcessingService.commandModeSystemPrompt,
                        rawTranscript: commandInput,
                        context: context,
                        customVocabulary: customVocabulary
                    )
                )
            } catch {
                os_log(.error, log: recordingLog, "Local Gemma edit mode failed: %{public}@", error.localizedDescription)
                return (selectedText, .commandModeFailedFallback(invocation: invocation), "")
            }
        }
        ...
        let systemPrompt = localGemmaSystemPrompt(
            customSystemPrompt: customSystemPrompt,
            outputLanguage: outputLanguage
        )
        ...
        do {
            ...
            os_log(.error, log: recordingLog, "Local Gemma post-processing failed: %{public}@", error.localizedDescription)
            return (trimmedRawTranscript, .postProcessingFailedFallback, "")
        }
    }

    private func localGemmaSystemPrompt(
        customSystemPrompt: String,
        outputLanguage: String
    ) -> String {
        var systemPrompt = Self.resolvedSystemPrompt(customSystemPrompt)
        let trimmedOutputLanguage = outputLanguage.trimmingCharacters(in: .whitespacesAndNewlines)
        if !trimmedOutputLanguage.isEmpty {
            systemPrompt += "\n\n输出语言：请将最终结果转换成\(trimmedOutputLanguage)，仅输出处理后的纯文本结果。"
        }
        return systemPrompt
    }

    private func localGemmaPromptForDisplay(
        systemPrompt: String,
        ...
        let postProcessingService = PostProcessingService(
            ...
            self.transcriptionTask = Task {
                ...
                do {
                    ...
                    let processResult: (finalTranscript: String, outcome: TranscriptProcessingOutcome, prompt: String)
                    let parsedTranscript: TranscriptCommandParsingResult
                    
                    switch self.asrEngineMode {
                    case .qwenNative:
                        // === Native Qwen3-ASR path ===
                        await MainActor.run { [weak self] in
                            self?.statusText = "Preparing Qwen3-ASR..."
                            self?.debugStatusMessage = "Preparing Qwen3-ASR model"
                            if self?.qwen3ASRLoadState != .ready {
                                self?.qwen3ASRLoadState = .loading(message: "Qwen3-ASR 模型准备中")
                            }
                        }
                        let asrResult = try await self.asrProvider.transcribe(audioURL: transcriptionFileURL)
                        await MainActor.run { [weak self] in
                            self?.qwen3ASRLoadState = .ready
                            self?.debugStatusMessage = String(
                                format: "Qwen3-ASR done in %.2fs, RTF %.3f",
                                asrResult.inferenceSeconds,
                                asrResult.rtf
                            )
                        }
                        ...
                        await MainActor.run { [weak self] in
                            self?.debugStatusMessage = "Running Gemma E4B correction"
                        }
                        let result = await self.processTranscriptWithLocalGemma(
                            parsedTranscript.transcript,
                            intent: sessionIntent,
                    ...
                    await MainActor.run {
                        ...
                        let trimmedFinalTranscript = processResult.finalTranscript.trimmingCharacters(in: .whitespacesAndNewlines)
                        let processingStatus = Self.statusMessage(
                            for: processResult.outcome,
                            parsedTranscript: parsedTranscript
                        )
                        self.lastPostProcessingPrompt = processResult.prompt
                        ...
                        self.recordPipelineHistoryEntry(
                            ...
                            postProcessingPrompt: processResult.prompt,
                        ...
                        case .postProcessingFailedFallback, .postProcessingConfigurationMissing:
