...
private struct SetupProviderSettingsSheet: View {
        ...
        VStack(spacing: 0) {
            ...
            ScrollView {
                VStack(alignment: .leading, spacing: 14) {
                    Text("第三方 API 模式会使用这里的 OpenAI-compatible 地址。留空的转录专用字段会自动沿用 API Base URL 和主 API key。")
                        .font(.caption)
                        .foregroundStyle(.secondary)

                    labeledSetupField("API Base URL", text: $apiBaseURLInput, placeholder: AppState.defaultProviderAPIBaseURL) {
                        appState.apiBaseURL = apiBaseURLInput.trimmingCharacters(in: .whitespacesAndNewlines)
                    }

                    labeledSetupField("Transcription API URL", text: $transcriptionAPIURLInput, placeholder: "留空则使用 API Base URL") {
                        appState.transcriptionAPIURL = transcriptionAPIURLInput.trimmingCharacters(in: .whitespacesAndNewlines)
                    }

                    labeledSetupSecureField("Transcription API Key", text: $transcriptionAPIKeyInput, placeholder: "留空则使用主 API key") {
                        appState.transcriptionAPIKey = transcriptionAPIKeyInput.trimmingCharacters(in: .whitespacesAndNewlines)
                    }
                }
                .padding(20)
            }
    ...
    private func labeledSetupField(
        _ title: String,
        text: Binding<String>,
        placeholder: String,
    ...
    private func labeledSetupSecureField(
        _ title: String,
        text: Binding<String>,
        placeholder: String,
        onSubmit: @escaping () -> Void
